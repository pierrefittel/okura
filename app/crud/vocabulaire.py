...
def get_due_cards(db: Session, limit: int = 50, list_id: int = None): # <-- Param list_id ajouté
    now = datetime.now()
    query = db.query(models.VocabCard).filter(models.VocabCard.next_review <= now)
    
    if list_id:
        query = query.filter(models.VocabCard.list_id == list_id) # <-- Filtrage optionnel
...
def add_cards_to_list_bulk(db: Session, list_id: int, cards_data: list[schemas.VocabCardCreate]):
    ...
    for card in cards_data:
        if card.ent_seq and card.ent_seq not in existing and card.ent_seq not in processed:
            defs = " | ".join(card.definitions) if card.definitions else ""
...
def add_card_to_list(db: Session, list_id: int, card_data: schemas.VocabCardCreate):
    defs = " | ".join(card_data.definitions) if card_data.definitions else ""
    c = models.VocabCard(list_id=list_id, ent_seq=card_data.ent_seq, terme=card_data.terme,
